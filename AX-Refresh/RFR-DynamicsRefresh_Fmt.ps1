...
$Script:Settings | Add-Member -Name Environment -Value '' -MemberType NoteProperty

Clear-Host
<#
$ParamDBServer = 'AAFESDEV-1' #Change SQL Server Name. Server\Instance, (local)
$($Script:Settings.DBName) = 'DynamicsAxTools' #Change DB Name (if you have changed during creation)
...
function Get-MainMenu
{
	...
	if ($Script:Settings.Environment) { "from ($($Script:Settings.Environment))" } else { "" }
	Write-Host '5. Restore Environment Configuration'
	Write-Host '6. Start AOS Services'
	Write-Host ''
	Write-Host '9. Additional AX Tools'
	Write-Host ''
	if ($Script:Settings.MachineName) { Get-MenuConfig }
	...
	switch ($Prompt.ToUpper().ToString()) {
		X {
			if ($Script:Settings.Environment) {
				RFR-VariableWiper
				Get-Menu
			}
			else {
		...
		Default {
			if (($Prompt -notlike "[LQ01234569]") -and !($Script:Settings.MachineName)) {
				$Script:WarningMsg = 'Invalid Option. Retry.'
				Get-Menu
			}
			elseif (($Prompt -notlike "[XLQ01234569]") -and ($Script:Settings.MachineName)) {
				$Script:WarningMsg = 'Invalid Option. Retry.'
				Get-Menu
			}
		}
...
function Get-MiscMenu
{
	...
	switch ($Prompt.ToUpper().ToString()) {
		X {
			if ($Script:Settings.MachineName) {
				RFR-VariableWiper
				Get-Menu
			}
			else {
		...
		Default {
			if (($Prompt -notlike "[LQR123456]") -and !($Script:Settings.MachineName)) {
				$Script:WarningMsg = 'Invalid Option. Retry.'
				Get-Menu
			}
			elseif (($Prompt -notlike "[XLQR123456]") -and ($Script:Settings.MachineName)) {
				$Script:WarningMsg = 'Invalid Option. Retry.'
				Get-Menu
			}
		}
...
function Get-BatchMenu
{
	...
	switch ($Prompt.ToUpper().ToString()) {
		...
		Default {
			if (($Prompt -notlike "[LQR123]") -and !($Script:Settings.MachineName)) {
				$Script:WarningMsg = 'Invalid Option. Retry.'
				Get-Menu
			}
			elseif (($Prompt -notlike "[XLQR123]") -and ($Script:Settings.MachineName)) {
				$Script:WarningMsg = 'Invalid Option. Retry.'
				Get-Menu
			}
		}
...
function Get-ServicesMenu
{
	...
	switch ($Prompt.ToUpper().ToString()) {
		...
		Default {
			if (($Prompt -notlike "[LQR1234]") -and !($Script:Settings.MachineName)) {
				$Script:WarningMsg = 'Invalid Option. Retry.'
				Get-Menu
			}
			elseif (($Prompt -notlike "[XLQR1234]") -and ($Script:Settings.MachineName)) {
				$Script:WarningMsg = 'Invalid Option. Retry.'
				Get-Menu
			}
		}
...
function Get-MenuConfig
{
	...
	Write-Host $Script:Settings.Environment -Fore Yellow
	Write-Host 'Machine Name: ' -NoNewline
	Write-Host $Script:Settings.MachineName -Fore Yellow
...
{
    $Script:Settings.Environment = (Read-Host "Enter Source Environment").ToUpper()
	if ($Script:Settings.Environment.Length -gt 30) {
	...
	$SqlConn.Open()
	$SqlQuery = “SELECT ENVIRONMENT FROM [AX_EnvironmentStore] WHERE ENVIRONMENT = '$($Script:Settings.Environment)' AND DELETED = 0"
	...
	if ($EnvCnt -ge 1) {
		$Prompt = Read-Host "Found Backup for $($Script:Settings.Environment). Load Configuration (Y/N)"
...
function Get-AOSConfiguration
{
    ...
    $Script:Settings | Add-Member -Name MachineName -Value $((Read-Host "Enter AOS Server Name").ToUpper()) -MemberType NoteProperty

	#$Script:MachineName = (Read-Host "Enter AOS Server Name").ToUpper()

	if ($Script:Settings.MachineName) {
		if ($Script:Settings.MachineName.Split('@').Count -eq 1) {
            $Script:Settings | Add-Member -Name MachineFullName -Value "01@$($Script:Settings.MachineName)" -MemberType NoteProperty
            $Script:Settings | Add-Member -Name MachineInstance -Value '01' -MemberType NoteProperty
		}
		elseif ($Script:Settings.MachineName.Split('@').Count -eq 2) {
            $Script:Settings | Add-Member -Name MachineFullName -Value $($Script:Settings.MachineName) -MemberType NoteProperty
            $Script:Settings | Add-Member -Name MachineInstance -Value $($Script:Settings.MachineName.Split('@')[0]) -MemberType NoteProperty
			$Script:Settings.MachineName = $Script:Settings.MachineName.Split('@')[1]
		}
	}
	else {
	...
	if (!(Test-Connection $Script:Settings.MachineName -Count 1 -Quiet)) {
		$Script:WarningMsg = 'Invalid AOS Server. Retry.'
		RFR-VariableWiper
		Get-Menu
	}
	...
	try {
		...
		$reg = [Microsoft.Win32.RegistryKey]::OpenRemoteBaseKey("LocalMachine",$Script:Settings.MachineName)
		$key = "SYSTEM\\CurrentControlSet\\services\\Dynamics Server\\$keyVersion\\$($Script:Settings.MachineInstance)"
		$regkey = $reg.opensubkey($key)
		$keyCurrent = $regKey.GetValue('Current')
		$key = "SYSTEM\\CurrentControlSet\\services\\Dynamics Server\\$keyVersion\\$($Script:Settings.MachineInstance)\\$keyCurrent"
		$regkey = $reg.opensubkey($key)
		...
		$SqlQuery = “INSERT INTO [AX_Environments] (ENVIRONMENT, DESCRIPTION, MACHINENAME, DBSERVER, DBNAME)
                        VALUES('$($Script:Settings.Environment)','$($Script:Settings.Environment)','$($Script:Settings.MachineName)','$Script:keyDbServer','$Script:keyDbName')"
		$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
...
function Get-RunningServers
{
	...
	$i = 1
	$DSServers.Tables[0] | ForEach-Object {
		...
		switch ($Prompt.ToUpper()) {
			Y {
				...
				if ($Script:HasServers) {
					$SqlQuery = “DELETE FROM [AX_Servers] WHERE ENVIRONMENT = '$($Script:Settings.Environment)' AND CREATEDDATETIME <= '$DateTime'"
					$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
					$SqlCommand.ExecuteNonQuery() | Out-Null
				}
				$DSServers.Tables[0] | ForEach-Object {
					if ($($Script:Settings.MachineFullName) -like "$($_.INSTANCE_NAME)@$($_.AOSID.substring(0,$_.AOSID.Length-5))") {
						$Active = '1'
					}
					else {
						$Active = '0'
					...
					$SqlQuery = “INSERT INTO [AX_Servers] (ENVIRONMENT, SERVERNAME, SERVERTYPE, ACTIVE, SERVERID, AOSID, INSTANCE, VERSION, STATUS)
                                    VALUES('$($Script:Settings.Environment)','$($_.AOSID.substring(0,$_.AOSID.Length-5))','AOS','$Active','$($_.ServerID)','$($_.AOSID)','$("$($_.INSTANCE_NAME)`@$($_.AOSID.substring(0,$_.AOSID.Length-5))")','$($_.VERSION)','$($_.STATUS)')"
					$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
					$SqlCommand.ExecuteNonQuery() | Out-Null
				}
				$SqlConn.Close()
...
function Get-RFREnviroments
{
	if (!($Script:Settings.Environment)) {
		...
		elseif ($Options.Count -eq $Prompt) {
			...
			$Script:Settings.Environment = (($RFREnvironments.Tables[0] | Select-Object Options -Unique)[$Prompt - 1]).Options
			RFR-LoadEnvironment
		}
	}
}
...
function RFR-LoadEnvironment
{
	...
	$SqlConn.Open()
	#
	$SqlQuery = “SELECT MACHINENAME FROM AX_Environments WHERE ENVIRONMENT = '$($Script:Settings.Environment)'"
	$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
	$Script:Settings.MachineName = $SqlCommand.ExecuteScalar()
	#
	$SqlQuery = “SELECT DBSERVER FROM AX_Environments WHERE ENVIRONMENT = '$($Script:Settings.Environment)'"
	$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
	$Script:keyDbServer = $SqlCommand.ExecuteScalar()
	#
	$SqlQuery = “SELECT DBNAME FROM AX_Environments WHERE ENVIRONMENT = '$($Script:Settings.Environment)'"
	$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
...
function RFR-StoreManager
{
	...
	if ($Backup) {
		...
		if ($Script:HasStore) {
			do {
				Write-Host ''
				if ($Script:IsJob) { $Prompt = 'Y' } else { $Prompt = Read-Host "Delete $($Script:Settings.Environment) Backups? (Y/N)" }
				switch ($Prompt.ToUpper()) {
					Y {
						RFR-DeleteStore
						Write-Host ''
						Write-Host "Exporting $($Script:Settings.Environment) to Env. Store." -Fore Green
	...
	if ($Restore) {
		Write-Host ''
		Write-Host "Importing $($Script:Settings.Environment) from Env. Store." -Fore Green
		$SqlConn = New-Object System.Data.SqlClient.SqlConnection
		$SqlConn.ConnectionString = "Server=$($Script:Settings.DBServer);Database=$($Script:Settings.DBName);Integrated Security=True"
		$SqlConn.Open()
		$SqlQuery = "SELECT ENVIRONMENT, SOURCEDBSERVER, SOURCEDBNAME, SOURCETABLE, RFRTABLENAME, COUNT, CREATEDDATETIME FROM AX_ENVIRONMENTSTORE WHERE ENVIRONMENT = '$($Script:Settings.Environment)' AND DELETED = 0"
		$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
		$Adapter = New-Object System.Data.SqlClient.SqlDataAdapter
		$Adapter.SelectCommand = $SqlCommand
	...
	if ($Check) {
		...
		$SqlConn.ConnectionString = "Server=$($Script:Settings.DBServer);Database=$($Script:Settings.DBName);Integrated Security=True"
		$SqlConn.Open()
		$SqlQuery = "SELECT COUNT(1) FROM AX_ENVIRONMENTSTORE WHERE ENVIRONMENT = '$($Script:Settings.Environment)' AND DELETED = 0"
		$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
		[boolean]$Script:HasStore = $SqlCommand.ExecuteScalar()
		#
		$SqlQuery = "SELECT COUNT(1) FROM AX_SERVERS WHERE ENVIRONMENT = '$($Script:Settings.Environment)'"
		$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
		[boolean]$Script:HasServers = $SqlCommand.ExecuteScalar()
		#
		$SqlQuery = "SELECT COUNT(1) FROM AX_ENVIRONMENTS WHERE ENVIRONMENT = '$($Script:Settings.Environment)'"
		$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
...
function Get-ScriptTable
{
	foreach ($Table in $Script:RFRTables) {
		$DestTable = "RFR_$($Script:Settings.Environment)`_$Table"
...
function RFR-DeleteStore
{
	...
	if ($Script:Settings.Environment) {
		...
		$SqlConn.ConnectionString = "Server=$($Script:Settings.DBServer);Database=$($Script:Settings.DBName);Integrated Security=True"
		$SqlConn.Open()
		$SqlQuery = "SELECT ENVIRONMENT, SOURCETABLE, RFRTABLENAME FROM [AX_EnvironmentStore] WHERE ENVIRONMENT = '$($Script:Settings.Environment)' AND DELETED = 0"
		$SqlCommand = New-Object System.Data.SqlClient.SqlCommand ($SqlQuery,$SqlConn)
		$Adapter = New-Object System.Data.SqlClient.SqlDataAdapter
		$Adapter.SelectCommand = $SqlCommand
		$EnvTables = New-Object System.Data.DataSet
		$Adapter.Fill($EnvTables) | Out-Null
		if ($Script:isJob) { $Prompt = 'Y' } else { $Prompt = Read-Host "Confirm Delete $($($Script:Settings.Environment))? (Y/N)" }
		switch ($Prompt.ToUpper()) {
			'Y' {
				Write-Host ''
				Write-Host "Deleting $($Script:Settings.Environment) from Env. Store." -Fore Green
				foreach ($Table in $EnvTables.Tables[0]) {
					$SqlQuery = “DROP TABLE [$($Table.RFRTABLENAME)]"
					$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
					$SqlCommand.ExecuteNonQuery() | Out-Null
				}
				$SqlQuery = "UPDATE [AX_EnvironmentStore] SET DELETED = 1, DELETEDDATETIME = '$DateTime' WHERE ENVIRONMENT = '$($Script:Settings.Environment)'"
				$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
				$SqlCommand.ExecuteNonQuery() | Out-Null
				#
				$SqlQuery = “DELETE FROM [AX_Servers] WHERE ENVIRONMENT = '$($Script:Settings.Environment)' AND CREATEDDATETIME < '$DateTime'"
				...
				#
				if ($HardDelete) {
					$SqlQuery = “DELETE FROM [AX_Environments] WHERE ENVIRONMENT = '$($Script:Settings.Environment)'"
					$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
					$SqlCommand.ExecuteNonQuery() | Out-Null
					#
					$SqlQuery = “DELETE FROM [AX_Servers] WHERE ENVIRONMENT = '$($Script:Settings.Environment)'"
					$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
					$SqlCommand.ExecuteNonQuery() | Out-Null
					#
					$SqlQuery = “DELETE FROM [AX_EnvironmentStore] WHERE ENVIRONMENT = '$($Script:Settings.Environment)'"
					$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
					$SqlCommand.ExecuteNonQuery() | Out-Null
					RFR-VariableWiper
				}
...
function Csv-ExportFile
{
	...
	## Create WorkFolder
	if (Test-Path -Path "$WorkFolder\$($Script:Settings.Environment)") {
		Remove-Item "$WorkFolder\$($Script:Settings.Environment)\*"
	}
	else {
		New-Item -ItemType directory -Path "$WorkFolder\$($Script:Settings.Environment)" | Out-Null
	}
	$SqlConn = New-Object System.Data.SqlClient.SqlConnection
	$SqlConn.ConnectionString = "Server=$Script:keyDbServer;Database=$Script:keyDbName;Integrated Security=True"
	$SqlConn.Open()
	...
	foreach ($Table in $Script:RFRTables) {
		...
		$Count += $i
		$DSInserts.Tables[0] | Export-Csv "$WorkFolder\$($Script:Settings.Environment)\$Table.csv" -Force -NoTypeInformation
...
function Csv-ImportFile
{
	...
	if (!(Test-Path "$WorkFolder\$($Script:Settings.Environment)\")) {
		$Script:WarningMsg = "Environment CSV Directory ($WorkFolder\$($Script:Settings.Environment)\) does not exist."
		Get-Menu
	}
	else {
		$MissingFile = $null
		foreach ($Table in $Script:RFRTables) {
			if (!(Test-Path "$WorkFolder\$($Script:Settings.Environment)\$Table.csv")) {
	...
	foreach ($Table in $AllTables) {
		...
		$SqlCommand.ExecuteNonQuery() | Out-Null
		if (Test-Path "$WorkFolder\$($Script:Settings.Environment)\$Table.csv") {
			$TotalCount += (Import-Csv "$WorkFolder\$($Script:Settings.Environment)\$Table.csv" | Measure-Object).Count
		}
	}
	$count = 0
	$i = 0
	foreach ($Table in $Script:RFRTables) {
		$TableCsv = Import-Csv "$WorkFolder\$($Script:Settings.Environment)\$Table.csv" | Select-Object Column1 -ExpandProperty Column1
...
function AOS-ServiceManager
{
	...
	$SqlConn.ConnectionString = "Server=$($Script:Settings.DBServer);Database=$($Script:Settings.DBName);Integrated Security=True"
	try {
		$SqlQuery = "SELECT INSTANCE FROM AX_Servers WHERE ENVIRONMENT = '$($Script:Settings.Environment)' AND ACTIVE = 1"
		$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
		$Adapter = New-Object System.Data.SqlClient.SqlDataAdapter
		$Adapter.SelectCommand = $SqlCommand
		$DSServers = New-Object System.Data.DataSet
	...
	try {
		$SqlQuery = "SELECT ServerID FROM RFR_$($Script:Settings.Environment)`_SYSSERVERCONFIG"
		$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
		$Adapter = New-Object System.Data.SqlClient.SqlDataAdapter
		$Adapter.SelectCommand = $SqlCommand
...
function RFR-RunAsJob
{
	...
	$SqlConn.ConnectionString = "Server=$($Script:Settings.DBServer);Database=$($Script:Settings.DBName);Integrated Security=True"
	$SqlConn.Open()
	$SqlQuery = “SELECT TOP 1 CREATEDDATETIME FROM AX_EnvironmentStore WHERE ENVIRONMENT = '$($Script:Settings.Environment)' ORDER BY CREATEDDATETIME DESC"
	$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
	...
	elseif ($RestoreDB) {
		...
		$SqlConn.ConnectionString = "Server=$($Script:Settings.DBServer);Database=$($Script:Settings.DBName);Integrated Security=True"
		$SqlConn.Open()
		$SqlQuery = “SELECT [BKPFOLDER] FROM [AX_Environments] WHERE ENVIRONMENT = '$($Script:Settings.Environment)'"
		$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
		$SQLBackup = $SqlCommand.ExecuteScalar()
		$SqlConn.Close()
		if ($SQLBckPath -notlike '') {
			if (Test-Path $SQLBckPath) {
				$Script:SQLBackup = Get-ChildItem -Path $SQLBckPath | Select-Object -First 1 | Where-Object { $_.Extension -match '.bak' } | Sort-Object -Property CreationTime -Descending
				AOS-ServiceManager -Stop
...
function Validate-Servers
{
	$SqlConn = New-Object System.Data.SqlClient.SqlConnection
	$SqlConn.ConnectionString = "Server=$($Script:Settings.DBServer);Database=$($Script:Settings.DBName);Integrated Security=True"
	$SqlQuery = "SELECT ServerName FROM AX_Servers WHERE ENVIRONMENT = '$($Script:Settings.Environment)'"
	$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
	$Adapter = New-Object System.Data.SqlClient.SqlDataAdapter
	$Adapter.SelectCommand = $SqlCommand
	$DSServers = New-Object System.Data.DataSet
	...
	if ($AOSServers) {
		...
		foreach ($Server in $AOSServers) {
			...
			$SqlQuery = "UPDATE [AX_Servers] SET ACTIVE = '$Active', IP = '$(($ServerIp.IPV4Address).IPAddressToString)', DOMAIN = '$($Computer.Domain)', FQDN = '$($FQDN)' WHERE ENVIRONMENT = '$($Script:Settings.Environment)' AND ServerName = '$Server'"
			$SqlCommand = New-Object System.Data.SqlClient.SqlCommand ($SqlQuery,$SqlConn)
			$SqlCommand.ExecuteNonQuery() | Out-Null
		}
		$SqlConn.Close()
...
function Update-SQLBKPFolder
{
	...
	if ($NewBkpFolder -and (Test-Path $NewBkpFolder)) {
		...
		$SqlConn.ConnectionString = "Server=$($Script:Settings.DBServer);Database=$($Script:Settings.DBName);Integrated Security=True"
		$SqlConn.Open()
		$SqlQuery = “UPDATE [AX_Environments] SET [BKPFOLDER] = '$NewBkpFolder' WHERE ENVIRONMENT = '$($Script:Settings.Environment)'"
		$SqlCommand = New-Object System.Data.SqlClient.SQLCommand ($SqlQuery,$SqlConn)
		$SQLBackup = $SqlCommand.ExecuteScalar()
		$SqlConn.Close()
	}
...
function Send-Email
{
	...
	$SMTPMessage.Subject = "$(if($Script:RFROk){"SUCCESS "} else {"FAILED "})Environment $($Script:Settings.Environment) has been refreshed on server $($Script:Settings.MachineFullName)."
...
function Write-Log
{
	...
	switch ($Level) {
		'Error' {
			$LogText = "ERROR: $(if($($Script:Settings.Environment)) {"($($Script:Settings.Environment)) "})$Message"
		}
		'Warn' {
			$LogText = "WARNING: $(if($($Script:Settings.Environment)) {"($($Script:Settings.Environment)) "})$Message"
		}
		'Info' {
			$LogText = "INFO: $(if($($Script:Settings.Environment)) {"($($Script:Settings.Environment)) "})$Message"
		}
	}
	$SqlConn = New-Object System.Data.SqlClient.SqlConnection
	$SqlConn.ConnectionString = "Server=$($Script:Settings.DBServer);Database=$($Script:Settings.DBName);Integrated Security=True"
...
function Get-RFRStart
{
	...
	RFR-SQLValidation $Script:Settings.DBServer $Script:Settings.DBName
	RFR-VariableWiper
	$DateTime = Get-Date
	...
	if ($Environment)
	{
        $Script:Settings.Environment = $Environment
